...
if __name__ == '__main__':
    ...
    board_setup = ['*n*q*b*r', '****p***', '********', '', '', '********', '****P***', '*N*Q*B*R']
    ...
    for row, each in enumerate(board_setup):
        ...
        for col, piece in enumerate(each): 
            setup_chess_pose = deepcopy(board_pose) 
            setup_chess_pose.position.x = board_pose.position.x + frame_dist + origin_piece + row * (2 * origin_piece)
            setup_chess_pose.position.y = board_pose.position.y - 0.55 + frame_dist + origin_piece + col * (2 * origin_piece)
            setup_chess_pose.position.z += 0.018

            piece_positionmap[str(row)+str(col)] = [setup_chess_pose.position.x, setup_chess_pose.position.y, setup_chess_pose.position.z-0.93] #0.93 to compensate Gazebo RViz origin difference   
